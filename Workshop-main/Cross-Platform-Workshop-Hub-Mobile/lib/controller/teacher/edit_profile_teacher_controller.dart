...
class EditWorkshopController extends GetxController {
  ...
  bool isStep1Completed() {
    return name.isNotEmpty &&
        ...
        endDate.value != null;
    // courseMediaInfos.isNotEmpty &&
  }

  bool isStep2Completed() {
    // Check if courseLocations is not empty and has the required fields
    return courseLocations.isNotEmpty &&
        courseLocations.every((location) =>
            location['schedule_Date'] != null && location['area'] != null);
  }


  bool isStep3Completed() {
    // Check if discountDTOS is not empty and has the required fields
  ...
  void showAllData() {
    ...
    // // print('\nStep 2 Data:');
    // for (var i = 0; i < courseMediaInfos.length; i++) {
    //   final mediaInfo = CourseMediaInfoDTO.fromJson(courseMediaInfos[i]);
    //   // print('Media Info $i: ${mediaInfo.urlImage}');
    // }
    // print('\nStep 2 Data:');
    // for (var i = 0; i < courseLocations.length; i++) {
    //   final location = CourseLocationDTO.fromJson(courseLocations[i]);
    //   // print(
